-- ============================================================================
-- Migration: 20260857000000_vault_notifications_and_formatting.sql
-- Description: Add 'vault' to notification_type enum and format currency as
--              GHS 20.00 in notification bodies.
-- ============================================================================

-- 1. Add 'vault' to notification_type enum
alter type public.notification_type add value if not exists 'vault';

-- 2. Update purchase_vault_with_balance with 'vault' type and 2-decimal formatting
...
begin
  ...
  -- Disallow duplicate active investment
  if exists (
...
-- 3. Update confirm_vault_payment
...
-- 4. Update claim_vault_investment
...
begin
  ...
  -- Notify user with 'vault' notification_type
  perform public.create_notification(
    v_inv.user_id,
    'vault'::public.notification_type,
    ...
    format('You successfully claimed %s points (GHS %s) from your matured %s Vault!',
           v_points::text,
           to_char(v_inv.expected_return_minor::numeric / 100.0, 'FM999,999,990.00'),
           v_inv.plan_name),
