-- ============================================================================
-- Migration 225 — disabling an account does not confiscate what it had
--
-- `credit_points` refused every entry type for a disabled account except
-- `admin_adjustment`. `redemption_refund` was not on that list, so declining a
-- withdrawal belonging to a disabled account raised "Account is disabled and
-- cannot earn" and the refund never landed.
--
-- The points had already left the balance when the withdrawal was REQUESTED.
-- Refusing to put them back does not stop anybody earning, it destroys money
-- somebody had already earned, at the exact moment an admin is making a
-- decision about their account.
--
-- The very next check in the same function already exempted
-- `redemption_refund` from the platform-wide earning pause, on exactly this
-- reasoning. The two exemptions should always have been the same list.
--
-- Caught by tests/money/redemption.test.ts, "reaches a disabled account:
-- disabling must not confiscate points".
-- ============================================================================
...
begin
  ...
  /*
    ⚠️ `redemption_refund` IS NOT EARNING, and it belongs in this exemption
    beside `admin_adjustment`.

    A withdrawal takes the points OUT of the balance when it is requested.
    Declining it puts them back. If the account was disabled in between, this
    check refused the refund and the points simply stopped existing: the user
    lost them for asking to be paid. Disabling an account is meant to stop it
    earning more, not to confiscate what it already had.

    The line below already exempted this entry type from the platform-wide
    pause for the same reason, and the two should have moved together.
  */
  if p_entry_type not in ('admin_adjustment', 'redemption_refund') then
...
/* ⚠️ A replace re-grants EXECUTE to PUBLIC. This function MINTS POINTS. It is
   restored to service_role alone, exactly as it was. */
revoke execute on function public.credit_points(
  uuid, bigint, public.ledger_entry_type, text, text, jsonb, boolean)
  from public, anon, authenticated;
grant execute on function public.credit_points(
  uuid, bigint, public.ledger_entry_type, text, text, jsonb, boolean)
  to service_role;
