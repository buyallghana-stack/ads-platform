-- ============================================================================
-- Migration: 20260860000000_fix_vault_balance_debit_reference.sql
-- Description: Fix debit_points reference_id in purchase_vault_with_balance
--              to use unique payment ID (vault_payments.id) instead of static
--              plan ID (vault_plans.id) to prevent duplicate key violations on
--              points_ledger_source_once_idx.
-- ============================================================================
...
begin
  ...
  -- Disallow duplicate active investment in the same plan
  if exists (
  ...
  if coalesce(v_user_balance, 0) < v_points then
    raise exception 'Insufficient balance. You need % points (GHS %) to purchase this plan, but have % points.',
      v_points::text,
      to_char(v_plan.price_minor::numeric / 100.0, 'FM999,999,990.00'),
      coalesce(v_user_balance, 0)::text
      using errcode = 'check_violation';
  end if;

  -- 1. Create confirmed payment record first to get a unique payment record ID
  ...
  -- 2. Deduct points from user balance using unique payment ID as reference
  -- (Using v_pay.id prevents duplicate key violations on points_ledger_source_once_idx)
  ...
  -- 5. Send notification with 'vault' notification_type and formatted currency string
