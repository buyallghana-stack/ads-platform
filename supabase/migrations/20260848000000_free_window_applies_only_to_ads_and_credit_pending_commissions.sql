-- ============================================================================
-- Migration 200 — free trial window applies only to ad watching, not referral commissions
--
-- BUG:
-- When William Jones purchased a plan, no referral commission was paid to the
-- referrer (admin@email.com).
--
-- ROOT CAUSE:
-- In `credit_points`, the 21-day free trial window check was written as:
--   `if p_entry_type not in ('admin_adjustment', 'redemption_refund', 'gift_code') then ...`
-- Since `admin@email.com`'s account creation was > 21 days ago, `credit_points`
-- threw an exception when attempting to credit `referral_purchase` commissions.
--
-- FIX:
-- 1. Update `credit_points` so that the 21-day trial limit ONLY gates direct
--    ad earnings (`ad_view`, `survey`), NOT referral commissions, purchase bonuses,
--    game rewards, or tasks.
-- 2. Retroactively pay all referral purchase commissions that were blocked.
-- ============================================================================
...
begin
  ...
  /*
    ...
    (or its grace period) lifts this immediately.
...
-- ---------------------------------------------------------------------------
-- 2. Retroactively process missed referral commissions for confirmed payments
-- ---------------------------------------------------------------------------
